...
export class CreateDiscountComponent {
  ...
  save(){
    ...
    this.discountService.createDiscounts(data).subscribe((resp:any) => {
      ...
      if(resp.message == 403){
        ...
        this.toastr.success("LA CAMPAÑA DE DESCUENTO SE REGISTRO CORRECTAMENTE","Exito");
  ...
  addProduct(){
    if(!this.product_id){
      this.toastr.error("Es necesario seleccionar un producto","Validación");
      return;
    }
    let INDEX = this.products_add.findIndex((prod:any) => prod.id == this.product_id);
    if(INDEX != -1){
      this.toastr.error("EL PRODUCTO YA EXISTE EN LA LISTA","Validación");
      return;
    }
  ...
  addCategorie(){
    if(!this.categorie_id){
      this.toastr.error("Es necesario seleccionar una categoria","Validación");
      return;
    }
    let INDEX = this.categories_add.findIndex((prod:any) => prod.id == this.categorie_id);
    if(INDEX != -1){
      this.toastr.error("LA CATEGORIA YA EXISTE EN LA LISTA","Validación");
      return;
    }
  ...
  addBrand(){
    if(!this.brand_id){
      this.toastr.error("Es necesario seleccionar una marca","Validación");
      return;
    }
    let INDEX = this.brands_add.findIndex((prod:any) => prod.id == this.brand_id);
    if(INDEX != -1){
      this.toastr.error("LA MARCA YA EXISTE EN LA LISTA","Validación");
      return;
    }
